...
def register(request):
    ...
    duplicate_email = User.objects.filter(email=request.POST["email"]).first()
    if duplicate_email:
        errors["duplicate_email"] = "Email is already registered"
    if len(errors) > 0:
...
def login(request):
    errors = {}
    registered = User.objects.filter(email=request.POST["email"]).first()
    if len(request.POST["password"]) < 8:
        errors["invalid_login"] = "Your login credentials are invalid"
        messages.error(request, errors["invalid_login"])
        return redirect("/")
    if registered:
        valid_pw = bcrypt.checkpw(
            request.POST["password"].encode(), registered.password.encode()
        )
        if valid_pw:
            request.session["user_id"] = registered.id
            request.session["user_name"] = registered.first_name
            return redirect("/wishes")
    else:
        errors["invalid_login"] = "Your login credentials are invalid"
        messages.error(request, errors["invalid_login"])
